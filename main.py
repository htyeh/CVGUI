...
option_menu.add_command(label="About", command=lambda: tkinter.messagebox.showinfo("About", "Credits:\n\nProgrammer: Yeh Hao-Tien ye.hao@protonmail.ch\nChinese corpus: GitHub: kaienfr\npyperclip: Al Sweigart al@inventwithpython.com"))
option_menu.add_separator()
option_menu.add_command(label="Read File", command=lambda: guikit.open_file(input_box))
option_menu.add_command(label="Encrypt File", command=lambda: guikit.enc_file(key_entry1, key_entry2, alphabet_index, runtime_label))
option_menu.add_command(label="Decrypt File", command=lambda: guikit.dec_file(key_entry1, key_entry2, alphabet_index, runtime_label))
option_menu.add_separator()
